...
see.numeric <- function(x) {
  ...
  Distinct.values <- length(table(x))# shows number of distinct values without NA
...
see.labelled <- function(x) {
  # make sure that those objects that have two classes and the first one is labelled and the second one is something else, that
  # before the following "if" loops are invoked, the generic for the second class is called. Only if there is only one class
  # specified the following "if" loops should be called
...
# to do: add class "see" to output, check if summary() returns the correct value

# Hmisc::describe has good example!!!!!!!

# Here's how you print a line over the length of the console! (from Hmisc print.describe<- )
#w <- paste(rep('-', .Options$width), collapse='')
#cat(w, '\n', sep='')

# writeLines() may be better than cat(), see here: https://stackoverflow.com/questions/4055762/in-r-can-i-stop-printcat-from-returning-null-and-why-does-catfoo-ret
